...
public class Specs {
	...
	public static var commands:Array = [
		...
		["move %n steps toward %m.spriteOrMouse",						" ", 1, "move:toward:",					10, "mouse-pointer"],
		["move %n steps toward ( %n , %n )",						" ", 1, "move:towardX:Y:",					10, 0, 0],
		["move %n steps at %n degrees",						" ", 1, "move:at:",					15],
		["--"],
		...
		["--"],
		["orbit %m.spriteOrMouse by %n degrees",	" ", 1, "orbitSprite:by:", "mouse-pointer", 15],
		["orbit ( %n , %n ) by %n degrees",			" ", 1, "orbitX:Y:by:", 0, 0, 15],
		["orbit %m.spriteOrMouse for %n steps",		" ", 1, "orbitSprite:for:", "mouse-pointer", 10],
		["orbit ( %n , %n ) for %n steps",			" ", 1, "orbitX:Y:for:", 0, 0, 10],
		["--"],
		...
		["--"],
		["draggable?",							"b", 1, "getDrag"],
		["make draggable",						" ", 1, "setDragTrue"],
		["make undraggable",					" ", 1, "setDragFalse"],
		...
		["-"],
		["shown?", 								"b", 2, "shown"],
		["%m.spriteOnly clone # %n shown?"	,	"b", 2, "cloneOf:no:visible"],
		["show",								" ", 2, "show"],
		["hide",								" ", 2, "hide"],
		["-"],
		...
		["-"],
		["%m.effect effect",					"r", 2, "effect", "ghost"],
		...
		["when I start as clone # %n",			"h", 6, "whenClonedAs:", 1],
		["when I start with clone # from %n to %n",		"h", 6, "whenClonedFrom:to:", 1, 2],
		["create clone of %m.spriteOnly",		" ", 6, "createCloneOf"],
		["create clone of %m.spriteOnly as %s",	" ", 6, "createCloneOf:as:"],
		["delete this clone",					"f", 6, "deleteClone"],
		["reset clones",						" ", 6, "resetClones"],
		["-"],
		...
		["touching %m.spriteOnly clone # %n?",	"b", 7, "touching:no:",			"", 1],
		...
		["-"],
		["ID",		"r", 7, "cloneIndex"],
		["count all sprites",								"r", 7, "countSprites"],
		["count sprites named %m.spriteOnly",								"r", 7, "countSpritesNamed:"],
		["%m.spriteOnly clone # %n exists?",				"b", 7, "clone:no:exists"],
		["-"],
		["turbo?", 								"b", 7, "turbo"],
		["turn Turbo Mode on",		" ", 7, "turboOn",],
		["turn Turbo Mode off",		" ", 7, "turboOff"],
		...
		["-"],
		["%m.case %s",						"r", 8, "case:on:",		"capitalize"],
		...
		["-"],
		["print %m.var",									" ", 9, "printTxt"],	
		["show %m.var print",								" ", 9, "showTxt"],	
		["hide %m.var print",								" ", 9, "hideTxt"],
		["-"],
		["set %m.var print size to %n",						" ", 9, "set:PrintSize:"],	
		["set %m.var print color to %c",					" ", 9, "set:PrintColor:"],
		["set %m.var position to x:%n y:%n",				" ", 9, "set:PrintX:Y:"],
		["-"],
		["set %m.var print width to %n",					" ", 9, "set:PrintWidth:"],
		["set %m.var print height to %n",					" ", 9, "set:PrintHeight:"],
		["set %m.var print alignment to %m.print",			" ", 9, "set:PrintAlign:"],
		["-"],
		["set %m.var print background to %c",				" ", 9, "set:PrintBackground:"],
		["show %m.var print background",					" ", 9, "show:PrintBackground"],
		["hide %m.var print background",					" ", 9, "hide:PrintBackground"],				
		["-"],
		["set %m.var print border to %c",					" ", 9, "set:PrintBorder:"],
		["show %m.var print border",						" ", 9, "show:PrintBorder"],
		["hide %m.var print border",						" ", 9, "hide:PrintBorder"],
		...
		["count %s in %m.list",								"r", 12, "count:in:"],
		["-"],
		...
		["d from (%n ,%n ) to (%n ,%n ) ",						"r", 13, "dFromX1:Y1:toX2:Y2:", 0, 0, 0, 0],
		["d from %m.spriteOnly to %m.spriteOnly",				"r", 13, "dFrom:to:"],
		["unit x component from %m.spriteOnly to %m.spriteOnly",		"r", 13, "uxFrom:to:"],
		["unit y component from %m.spriteOnly to %m.spriteOnly",		"r", 13, "uyFrom:to:"],
		["unit x component from (%n ,%n ) to (%n ,%n ) ",		"r", 13, "uxFromX1:Y1:toX2:Y2:", 0, 0, 0, 0],
		["unit y component from (%n ,%n ) to (%n ,%n ) ",		"r", 13, "uyFromX1:Y1:toX2:Y2:", 0, 0, 0, 0],
		["x component of %n @ %d.direction",		"r", 13, "xCompOf:at:", 25, 45],
		["y component of %n @ %d.direction",		"r", 13, "yCompOf:at:", 25, 45],
		["direction from (%n ,%n )  to (%n ,%n ) ",				"r", 13, "dirFromX1:Y1:toX2:Y2:", 0, 0, 0, 0],
		["direction from %m.spriteOnly to %m.spriteOnly",		"r", 13, "dirFrom:to:"],
		["direction for Δx: %n Δy: %n",		"r", 13, "dirForX:Y:"],
		["set %m.attribute of %m.spriteOnly clone # %n to %s",	" ", 13, "setAttr:of:no:to:", "x position", "_myself_" , 1, 0],
		["add tag %s",	" ", 13, "addTag:", ""],
		["delete tag %s",	" ", 13, "deleteTag:", ""],
		["add tag %s to %m.spriteOnly clone # %n",	" ", 13, "addTag:to:no:", "", "_myself_", 1],
		["delete tag %s from %m.spriteOnly clone # %n",	" ", 13, "deleteTag:from:no:", "", "_myself_", 1],
		["tag # %n",	"r", 13, "getTagNo:", 1],
		["count tags",	"r", 13, "countTags"],
		["tag # %n of %m.spriteOnly clone # %n",	"r", 13, "getTagNo:of:clone:", 1, "_myself_", 1],
		["count tags of %m.spriteOnly clone # %n",	"r", 13, "countTagsOf:clone:", "_myself_",1],
		["has tag %s?",	"b", 13, "hasTag:"],
		["%m.spriteOnly clone # %n has tag %s?",	"b", 13, "clone:no:hasTag:", "_myself_",1, ""],
